 ...
 * Описание задачи:
 * ================
 * Модифицируйте программу-калькулятор из главы 7, чтобы поток ввода стал явным
 * параметром (как показано в разделе 8.5.8). Кроме того, напишите конструктор
 * класса Token_stream (раздел 7.8.2) с параметром istream& так, чтобы, когда вы
 * поймете, как создавать собственные потоки ввода и вывода (например, подключать
 * файлы), вы смогли бы использовать калькулятор с ними. Указание: не пытайтесь
 * копировать istream.
 *
 * Идея реализации:
 * ================
 * Переменна ts из глобальной области видимости была перенесена в локальную.
 * Инициализация произведена в функции main.
 *
 * В Token_stream изменен конструктор. Теперь он принимает поток, откуда считывать
 * лексемы.
 * */
...
const string pow_fun = "pow";        // Ключевое слово возведения в степень
...
public:
    ...
    Token_stream(istream &istr) :is(istr) { };
...
private:
    ...
    Token buffer {0};             // хранит лексему, возвращенную вызовом putback()
    istream &is;              // Выбранный поток
};
...
double expression(Token_stream &);        // объявление чтобы primary() мог вызвать expression()
...
double pow_funct(Token_stream &ts) {
    Token t2 = ts.get();
    // Считали скобку
    if (t2.kind != '(') error("");
    double num = expression(ts);
    // Считали запятую
    t2 = ts.get();
    if (t2.kind != ',') error("sqr: a comma was omitted");
    double degree = expression(ts);
...
double primary(Token_stream &ts)
{
    ...
    switch (t.kind) {
        ...
        default:
            ...
            if (t.name == pow_fun) return pow_funct(ts);
...
{
    double left = term(ts);      // чтение и вычисление Term
    Token t = ts.get();        // получить следующий токен из потока

    while(true) {
        switch(t.kind) {
            case '+':
                left += term(ts);    // вычислить term() и сложить с числом
                t = ts.get();
                break;
            case '-':
                left -= term(ts);    // вычислить term() и вычесть из числа
                t = ts.get();
                break;
            default:
                ts.putback(t);     // вернуть t обратно в поток
...
double declaration(Token_stream &ts, bool const_val)
...
int main() {
    try {
        ...
        calculate(ts);        // Функция вычисления выражений
