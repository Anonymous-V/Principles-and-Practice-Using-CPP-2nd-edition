 ...
 * Описание задачи:
 * ================
 * Реализуйте именованные константы, которые не могут изменять свои значения.
 * Указание: в класс Variable необходимо добавить член, позволяющий различать
 * константы и переменные, и проверять его в функции set_value(). Если хотите дать
 * пользователю возможность объявлять собственные именованные константы (а не
 * ограничиваться предопределенными константами pi и e), то необходимо добавить
 * соответствующее обозначение, например const pi = 3.14;.
 *
 * Идея реализации:
 * ================
 * Для реализации этого подхода требуется немного видоизменить функцию
 * define_name, добавив в нее ещё один передаваемый аргумент - булево значение,
 * которое в значении true означает, что переменная является константой, иначе -
 * обычной переменной. И, соответственно, немного видоизменить класс Variable и
 * функцию set_value.
 * */
    ...
    switch (t.kind) {
        ...
        default:
            ...
            if (t2.kind == letsym) {
                ...
                if ( !is_declared(t.name) ) error(t.name, " isn't defined");
...
double define_name(string var, double val, bool const_val)
...
// заданным Выражением
{
    ...
    double d = expression();
    define_name(var_name, d, const_val);
    return d;
}
