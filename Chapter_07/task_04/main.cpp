 ...
 * Описание задачи:
 * ================
 * Функции get_value(), set_value(), is_declared() и define_name() работают с
 * переменной var_table. Определите класс Symbol_table с членом var_table типа
 * vector<Variable> и функциями-членами get(), set(), is_declared() и define().
 * Перепишите калькулятор с использованием переменной типа Symbol_table.
 *
 * Идея реализации:
 * ================
 * Из описания задачи ясно, что необходимо создать класс и перенести функции в
 * этот класс. При таком подходе код стал немного чище.
 * */
