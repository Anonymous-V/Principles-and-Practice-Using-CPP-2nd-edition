 ...
 * Описание задачи:
 * ================
 * Модифицируйте калькулятор так, чтобы он работал только с целыми числами;
 * предусмотрите ошибки, возникающие при потере точности и переполнении.
 * Указание: воспользуйтесь narrow_cast(раздел 7.5).
 *
 * Идея реализации:
 * ================
 * Т.к. все данные проходят через функцию-член get, класса Token_stream, то
 * проверку достаточно провести именно в нем, добавив narrow_cast при получении
 * числа из потока. Таким образом можно будет использовать заранее определенные
 * переменные типа, pi, но определять новые вещественного типа нельзя.
 * Также можно было бы заменить везде типы double на int, но в качестве экономии
 * времени и возможностью быстрого возвращения к типу double это не было реализовано.
 * */
