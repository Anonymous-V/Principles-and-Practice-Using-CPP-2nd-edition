 ...
 * после её объявления с помощью инструкции let. Подумайте, почему эта возможность
 * может быть полезной и к каким проблемам она может привести.
 *
 * Идея реализации:
 * ================
 * Т.к. в предыдущей реализации let было заменено на #, то пусть символ объявления
 * переменных и останется #.
 * Для того, чтобы можно было в существующей переменной заменить значение,
 * необходимо в функции primary() в разделе default дописать условие проверки на
 * ввод переменной.
 * */
