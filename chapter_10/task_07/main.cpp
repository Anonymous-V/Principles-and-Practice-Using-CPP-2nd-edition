 ...
 * История версий:
 *
 * 27.09.2020 - переписан Виталием под римские числа
 ...
 * Идея реализации:
 * ================
 * Для реализации римского калькулятора досточно было в функцию
 * Token Token_stream::get() добавить проверку на введенное римское число
 * (реализовано в секции default). Если эта проверка успешна, то необходимо было
 * перевести римское число в обычное десятичное и применить операции над десятичным
 * числом.
 * В итоге получили действующий римский калькулятор, доработав только одну функцию.
 * */
...
bool check_roman(string& roman)
// Является ли строка римским числом
{
    for (char sym : roman)
        if (!Roman::is_roman(sym)) return false;
    return true;
}
...
double primary()
{
    ...
    switch (t.kind) {
        ...
        default:
            ...
            if (t.name == pow_fun) return pow_funct();
    ...
    while (cin)
        try {
            ...
            if (t.kind == help) write_help();     // Вызов справки
                ...
                ts.putback(t);
                int res = statement();            // В римских числах нет вещественных
                cout << result << Roman::to_roman(res) << " (" << res << ")" << endl;
            }
        } catch (exception &e) {
